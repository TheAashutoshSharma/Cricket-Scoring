...
function App() {
  ...
  // Inline editing state: { team:"A"|"B", type:"player"|"bowler", idx:number }
  ...
  function startEdit(team, type, idx, currentName) {
    ...
    setTimeout(() => { if (editInputRef.current) editInputRef.current.focus(); }, 50);
  }
  function commitEdit() {
  ...
  function addRuns(r, extra) {
    ...
    setMatch(prev => {
      ...
      if (!extra) {
        ...
        m.extras[bt]+=r;
        if(extra==="Wide")    m.extrasBreakdown[bt].wide+=r;
        if(extra==="No Ball") m.extrasBreakdown[bt].noBall+=r;
        if(extra==="Bye")     {m.extrasBreakdown[bt].bye+=r;    bT.players[b1].balls++;}
        if(extra==="Leg Bye") {m.extrasBreakdown[bt].legBye+=r; bT.players[b1].balls++;}
        wT.bowlers[bi].runs+=r;
      }
  ...
  function addWicket(how) {
    setMatch(prev => {
      ...
      if (how===RET_HURT) {
        ...
        // Find next player who isn't out and isn't one of the current batsmen
      ...
      if(m.balls[bt]===6){m.overs[bt]++;m.balls[bt]=0;wT.bowlers[bi].overs++;wT.bowlers[bi].balls=0;}
  ...
  function BatterCard({editable}) {
    return (
      <div style={S.card}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 36px 36px 36px 48px",gap:4,marginBottom:6}}>
          {["NAME","R","B","4s","SR"].map(h=><span key={h} style={{color:"#334155",fontSize:10,textAlign:h==="NAME"?"left":"center"}}>{h}</span>)}
        </div>
        {[
          ...
          <div key={si} style={{display:"grid",gridTemplateColumns:"1fr 36px 36px 36px 48px",gap:4,padding:"7px 0",borderTop:"1px solid #0f172a",alignItems:"center"}}>
      ...
      <div style={S.wrap}>
        ...
        {!match.inningsOver[bt]&&(
          <div style={{padding:"0 12px"}}>
            ...
            {/* EXTRAS */}
            <div style={S.card}>
              <div style={S.lbl}>EXTRAS (+1 run)</div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6}}>
                {EXTRAS.map(ex=>(
                  <button key={ex} onClick={()=>addRuns(1,ex)}
                    style={{padding:"11px 0",borderRadius:10,border:"1px solid #4c1d95",background:"rgba(109,40,217,.12)",color:"#a78bfa",fontWeight:"bold",fontSize:11,cursor:"pointer",touchAction:"manipulation",fontFamily:"Georgia,serif"}}>
                    {ex}
                  </button>
                ))}
              </div>
