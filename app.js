...
function ScorecardInnings({ match, inn, team, opp }) {
  ...
  return (
    <div style={{background:SP.bg3,borderRadius:12,overflow:"hidden",border:"1px solid rgba(73,72,71,.25)",marginBottom:16}}>
      ...
      {/* ── Ball-by-ball log ── */}
      {(()=>{
        var log = (match.ballLog&&match.ballLog[inn])||[];
        if (!log.length) return null;
        // Build overs from the full innings log
        var overs = [];
        var cur = [];
        var legalInCur = 0;
        log.forEach(b => {
        ...
        // Show last 3 overs (latest first)
        var displayOvers = overs.slice(-3);
        var totalCompleted = match.overs[inn] || 0;
        ...
        return (
          <div style={{padding:"10px 14px",borderTop:"1px solid rgba(73,72,71,.15)"}}>
            <div style={{color:SP.textDim,fontSize:10,fontWeight:"700",letterSpacing:1,marginBottom:8}}>BALL LOG</div>
            <div style={{overflowX:"auto",WebkitOverflowScrolling:"touch",paddingBottom:4}}>
              <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"nowrap",minWidth:"max-content",paddingLeft:2}}>
                {items.map((item,i) => item.type==="label" ? (
                  ...
                  <div key={"b"+i} style={{
                    ...
                    background:bBg(item.b), flexShrink:0,
